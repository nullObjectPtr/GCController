...
@implementation GCControllerConverters
...
+(const void**) NSArrayToRetainedCArray:(NSArray<id>*) array
{
    ...
    long _count = [array count];
    const void** _buffer = (const void**) malloc(sizeof(void*) * _count);
...
+(const void**) NSSetToRetainedCArray:(NSSet<id>*) set
{
    if(set == nil)
        return nil;
    
    long _count = [set count];
    const void** _buffer = (const void**) malloc(sizeof(void*) * _count);
...
+(const long*) ToTypeIdArray:(NSArray<id>*) array
{
    ...
    long i = 0;
    for (id item in array) {
        long typeId = [GCControllerConverters GetTypeId:item];
        _buffer[i] = typeId;
        i++;
    }
